...
export async function fetchIncidents(options?: {
    ...
    try {
        ...
        // If no data from API, fallback to mock but log it
        if (!data || !data.incidents || data.incidents.length === 0) {
            console.info('No live incidents found, using simulation dataset.')
            return generateMockIncidents(30)
        }

        return data.incidents.map(mapDBIncidentToSecurityIncident)
    } catch (error) {
        console.warn('API connection failed, falling back to offline simulation:', error)
        return generateMockIncidents(30)
    }
}
