...
async function main() {
    ...
    for (const incident of incidents) {
        const { type, county, targetAsset, attackVector, severity, ...baseData } = incident;
        await prisma.incident.create({
            data: {
                ...baseData,
                status: (baseData.status as any) || 'ACTIVE',
                severity: (severity as any) || 'MEDIUM',
                createdById: admin.id,
                encryptedDetails: JSON.stringify({ type, county, targetAsset, attackVector })
            },
        })
    }
